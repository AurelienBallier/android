...
static char wcnss_macAddr[WLAN_MAC_ADDR_SIZE*3];
module_param_string(mac, wcnss_macAddr, WLAN_MAC_ADDR_SIZE*3, 0);
static void wcnss_wlan_init_mac(void)
{
	int i, j;
	short byte1, byte0;
	...
	if (strlen(wcnss_macAddr) < WLAN_MAC_ADDR_SIZE*3-1)
		return;

	for (i=0, j=0; i < WLAN_MAC_ADDR_SIZE; i++, j+=3) {
		if ((byte1 = hex_to_bin(*(wcnss_macAddr + j))) < 0)
			return;

		if ((byte0 = hex_to_bin(*(wcnss_macAddr + j+1))) < 0)
			return;

		penv->wlan_nv_macAddr[i] = (char)(byte1*16 + byte0);
	}

	pr_info("Write MAC Addr:" MAC_ADDRESS_STR "\n",
		penv->wlan_nv_macAddr[0], penv->wlan_nv_macAddr[1],
		penv->wlan_nv_macAddr[2], penv->wlan_nv_macAddr[3],
		penv->wlan_nv_macAddr[4], penv->wlan_nv_macAddr[5]);
}
